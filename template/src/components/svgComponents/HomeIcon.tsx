import useThemeColors from '@src/customHooks/useThemeColors';
import Svg, { Path, SvgProps } from 'react-native-svg';

interface HomeIconProps extends SvgProps {
  size: number;
  focused: boolean;
}

const filledPath =
  'M7.825.12a.5.5 0 0 0-.65 0L0 6.27v7.23A1.5 1.5 0 0 0 1.5 15h4a.5.5 0 0 0 .5-.5v-3a1.5 1.5 0 0 1 3 0v3a.5.5 0 0 0 .5.5h4a1.5 1.5 0 0 0 1.5-1.5V6.27z';
const outlinePath =
  'm7.5.5.325-.38a.5.5 0 0 0-.65 0zm-7 6-.325-.38L0 6.27v.23zm5 8v.5a.5.5 0 0 0 .5-.5zm4 0H9a.5.5 0 0 0 .5.5zm5-8h.5v-.23l-.175-.15zM1.5 15h4v-1h-4zm13.325-8.88-7-6-.65.76 7 6zm-7.65-6-7 6 .65.76 7-6zM6 14.5v-3H5v3zm3-3v3h1v-3zm.5 3.5h4v-1h-4zm5.5-1.5v-7h-1v7zm-15-7v7h1v-7zM7.5 10A1.5 1.5 0 0 1 9 11.5h1A2.5 2.5 0 0 0 7.5 9zm0-1A2.5 2.5 0 0 0 5 11.5h1A1.5 1.5 0 0 1 7.5 10zm6 6a1.5 1.5 0 0 0 1.5-1.5h-1a.5.5 0 0 1-.5.5zm-12-1a.5.5 0 0 1-.5-.5H0A1.5 1.5 0 0 0 1.5 15z';

const HomeIcon = ({
  size,
  width,
  height,
  focused,
  ...props
}: HomeIconProps) => {
  const colors = useThemeColors();

  return (
    <Svg
      width={size ?? width}
      height={size ?? height}
      viewBox="0 0 15 15"
      fill="none"
      color={focused ? colors.ON_PRIMARY : colors.DISABLE_GRAY}
      {...props}
    >
      <Path d={focused ? filledPath : outlinePath} fill="currentColor" />
    </Svg>
  );
};

export default HomeIcon;
